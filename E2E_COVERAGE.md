...
| --- | --- | --- | --- | --- |
| `e2e/auth/auth.spec.ts` | 5 | 5 | 0 | All browsers |
| `e2e/smoke.spec.ts` | 2 | 2 | 0 | All browsers |
| `e2e/i18n.spec.ts` | 5 | 5 | 0 | All browsers |
| `e2e/trino/connection-form.spec.ts` | 10 | 10 | 0 | All browsers |
| `e2e/trino/catalog-browser.spec.ts` | 8 | 8 | 0 | All browsers |
| `e2e/trino/trino.spec.ts` | 9 | 9 | 0 | All browsers |
| `e2e/trino/trino-tabs.spec.ts` | 10 | 10 | 0 | All browsers |
| `e2e/storage/connection.spec.ts` | 6 | 6 | 0 | Requires local Garage instance |
| `e2e/storage/browsing.spec.ts` | 3 | 3 | 0 | Requires local Garage instance |
| `e2e/storage/upload.spec.ts` | 2 | 2 | 0 | Requires local Garage instance |
| `e2e/storage/delete.spec.ts` | 3 | 3 | 0 | Requires local Garage instance |
| `e2e/storage/preview.spec.ts` | 5 | 5 | 0 | Requires local Garage instance |
| `e2e/storage/context-menu.spec.ts` | 3 | 3 | 0 | Requires local Garage instance |
| `e2e/storage/download.spec.ts` | 2 | 2 | 0 | Requires local Garage instance |
| `e2e/storage/permissions.spec.ts` | 3 | 3 | 0 | Requires local Garage instance |
| `e2e/storage/recent-items.spec.ts` | 2 | 2 | 0 | Requires local Garage instance |

**Total: 78 tests / 78 pass** <!-- markdownlint-disable-line MD036 -->
...
**File:** `e2e/trino/connection-form.spec.ts`
...
**File:** `e2e/trino/catalog-browser.spec.ts`
...
**File:** `e2e/trino/trino-tabs.spec.ts`
...
**File:** `e2e/storage/connection.spec.ts`
...
| 3 | `disconnects from Garage S3` | StorageConnectForm disconnect | — |
| 4 | `saves a connection and reconnects from the saved connections list` | StorageConnectForm (saved connections) | `ListBuckets` |
| 5 | `forgets a saved connection via the remove button` | StorageConnectForm (forget dialog) | — |
| 6 | `clicking a bucket tile in the grid navigates to the bucket explorer` | BucketGrid, FileExplorer | `ListObjectsV2` |

#### Browsing & Navigation

**File:** `e2e/storage/browsing.spec.ts`
...
**File:** `e2e/storage/upload.spec.ts`

| # | Test | Components exercised | S3 operations |
| --- | --- | --- | --- |
| 10 | `uploads realistic faker-generated text, csv, and image files` | UploadModal, TextPreview, CsvPreview, ImagePreview | `PutObject` × 3, `HeadObject`, `GetObject` |
| 11 | `uploads files and resolves replace, skip, and rename conflicts` | UploadModal (replace/skip/rename flow) | `PutObject` × 3, `GetObject` × 5 |

#### Delete & Selection

**File:** `e2e/storage/delete.spec.ts`

| # | Test | Components exercised | S3 operations |
| --- | --- | --- | --- |
| 12 | `deletes files and folders recursively` | SelectionToolbar, DeleteConfirmModal | `DeleteObjects`, `HeadObject` |
| 13 | `cancel delete does not remove the selected file` | DeleteConfirmModal (cancel path) | `HeadObject` (verify) |
| 14 | `selects all items and deselects them with the header checkbox` | ObjectTable (select-all checkbox), SelectionToolbar | — |

#### Preview

**File:** `e2e/storage/preview.spec.ts`

| # | Test | Components exercised | S3 operations |
| --- | --- | --- | --- |
| 15 | `previews text files from the object table` | PreviewModal, TextPreview (double-click row) | `/storage/api/preview` |
| 16 | `shows fallback preview for a known binary file type` | PreviewModal, FallbackPreview (`X-Preview-Renderable: false` path) | `/storage/api/preview` |
| 17 | `shows binary fallback preview for non-decodable binary content` | PreviewModal, FallbackPreview (client-side binary detection) | `/storage/api/preview` |
| 18 | `shows a PDF preview for PDF files` | PreviewModal, PdfPreview | `/storage/api/preview` |
| 19 | `closes preview modal when the Escape key is pressed` | PreviewModal (Escape key dismiss) | `/storage/api/preview` |

#### Context Menu

**File:** `e2e/storage/context-menu.spec.ts`

| # | Test | Components exercised | S3 operations |
| --- | --- | --- | --- |
| 20 | `context menu appears on right-click and shows file actions` | ContextMenu (Preview, Download, Delete items) | — |
| 21 | `context menu closes when the Escape key is pressed` | ContextMenu (keyboard dismiss) | — |
| 22 | `deletes a single file via context menu` | ContextMenu → DeleteConfirmModal | `DeleteObjects` |

#### Downloads

**File:** `e2e/storage/download.spec.ts`

| # | Test | Components exercised | S3 operations |
| --- | --- | --- | --- |
| 23 | `downloads a file via the selection toolbar` | SelectionToolbar (Download button) | `HEAD /storage/api/download`, `GET /storage/api/download` |
| 24 | `downloads a file via the context menu` | ContextMenu (Download action) | `HEAD /storage/api/download`, `GET /storage/api/download` |

#### Permissions / Error Scenarios

**File:** `e2e/storage/permissions.spec.ts`

| # | Test | Components exercised | S3 operations | Requires admin |
| --- | --- | --- | --- | --- |
| 25 | `surfaces real Garage permission errors for a read-only bucket` ⚙ | UploadModal (access-denied error), toast system, delete error | `PutObject` (403), `DeleteObjects` (403) | Yes |
| 26 | `shows access denied error when browsing a write-only bucket` ⚙ | `+error.svelte` (403 page) | `ListObjectsV2` (403) | Yes |
| 27 | `shows access denied error when browsing a no-access bucket` ⚙ | `+error.svelte` (403 page) | `ListObjectsV2` (403) | Yes |

#### Recent Items

**File:** `e2e/storage/recent-items.spec.ts`
...
| `createCsvUploadFixture` | `.csv` | `text/csv` | 5 rows: customer, email, city, annual\_spend |
...
| --- | --- | --- | --- |
| `StorageConnectForm` | `src/lib/components/storage/StorageConnectForm.svelte` | Yes | Connect, invalid creds, saved connections, forget dialog, disconnect |
| `BucketGrid` | `src/lib/components/storage/BucketGrid.svelte` | Yes | Bucket list rendered; tile click navigates to explorer |
| `FileExplorer` | `src/lib/components/storage/FileExplorer.svelte` | Yes | Selection mode, navigation, pagination, page-size change, toolbar |
| `ObjectTable` | `src/lib/components/storage/ObjectTable.svelte` | Yes | File rows, folder rows, select-all checkbox, indeterminate state |
| `FileRow` | `src/lib/components/storage/FileRow.svelte` | Yes | Double-click preview, single-click selection, right-click context menu |
| `FolderRow` | `src/lib/components/storage/FolderRow.svelte` | Yes | Click to navigate, selection checkbox, right-click context menu |
| `StorageBreadcrumb` | `src/lib/components/storage/StorageBreadcrumb.svelte` | Yes | Breadcrumb segments, click to navigate up |
| `ContextMenu` | `src/lib/components/storage/ContextMenu.svelte` | Yes | Right-click open, Preview/Download/Delete items, Escape to close |
| `PreviewModal` | `src/lib/components/storage/PreviewModal.svelte` | Yes | Open, close (button + Escape), loading, text/csv/image/pdf/fallback content |
| `TextPreview` | `src/lib/components/storage/preview/TextPreview.svelte` | Yes | Text file content rendered |
| `CsvPreview` | `src/lib/components/storage/preview/CsvPreview.svelte` | Yes | CSV table rendered |
| `ImagePreview` | `src/lib/components/storage/preview/ImagePreview.svelte` | Yes | SVG rendered with alt text |
| `PdfPreview` | `src/lib/components/storage/preview/PdfPreview.svelte` | Yes | PDF iframe rendered |
| `FallbackPreview` | `src/lib/components/storage/preview/FallbackPreview.svelte` | Yes | Known-binary and client-detected binary paths; "Download full file" link |
| `UploadModal` | `src/lib/components/storage/UploadModal.svelte` | Yes | Upload, conflict resolution (replace/skip/rename), permission error |
| `DeleteConfirmModal` | `src/lib/components/storage/DeleteConfirmModal.svelte` | Yes | Confirm (single file, folder), cancel (no deletion) |
| `SelectionToolbar` | `src/lib/components/storage/SelectionToolbar.svelte` | Yes | Delete button, Download button |
| `Pagination` | `src/lib/components/Pagination.svelte` | Yes | Next/prev/page indicator, page-size selector |
| `RecentItems` | `src/lib/components/storage/RecentItems.svelte` | Yes | Recent Files tab, Recent Locations tab |
| `TimestampDisplay` | `src/lib/components/storage/TimestampDisplay.svelte` | Partial | Rendered but not directly asserted |

### Server Routes

| Route | Method | E2E covered? | What's tested |
| --- | --- | --- | --- |
| `/storage` | GET | Yes | Bucket list, connect form |
| `/storage/[bucket]/[...prefix]` | GET | Yes | Object listing, pagination, 403 error page |
| `/storage/api/upload` | POST | Yes | Successful upload, permission error |
| `/storage/api/download` | GET | Yes | File download triggered via toolbar and context menu |
| `/storage/api/download` | HEAD | Yes | Pre-flight check invoked by download action |
| `/storage/api/delete` | DELETE | Yes | Files, recursive folders, permission error |
| `/storage/api/preview` | GET | Yes | Text, CSV, image, PDF, binary-fallback, unknown-binary previews |

### Server-Side Services

| Service / File | E2E covered? | Notes |
| --- | --- | --- |
| `src/lib/server/storage/service.ts` | Yes | All exported operations exercised |
| `src/lib/server/storage/s3-provider.ts` | Yes | listBuckets, listObjects, upload, delete, download, preview |
| `src/lib/server/storage/s3-client.ts` | Yes | Client creation with test credentials |
| `src/lib/server/storage/s3-errors.ts` | Yes | 403 error mapped to user-facing message |
| `src/lib/server/storage/user-connections.ts` | Yes | Connect, disconnect, per-user state |
| `src/lib/server/storage/preview/binary.ts` | Yes | `KNOWN_BINARY_TYPES` path (zip) and renderable:false header |
| `src/lib/server/storage/preview/stream.ts` | Yes | Text, CSV, image, PDF streaming |
| `src/lib/server/storage/hdfs-provider.ts` | No | HDFS stub not implemented or tested |
| `src/lib/server/storage/preview/parquet.ts` | No | Parquet preview disabled |
...
| --- | :---: | :---: | :---: | --- |
| **Full access** | Yes | Yes | Yes | Tests 1–24, 28–29: browse, upload, delete, preview, paginate |
| **Read-only** | No | Yes | No | Test 25: upload blocked (403), delete blocked (403) |
| **Write-only** | No | No | Yes | Test 26: listing returns 403 error page |
| **No access** | No | No | No | Test 27: listing returns 403 error page |

### Permission Scenarios Not Yet Covered

| Scenario | `owner` | `read` | `write` | What to test |
| --- | :---: | :---: | :---: | --- |
| **Write-only — upload succeeds** | No | No | Yes | Upload itself should succeed (listing is blocked; this is not yet verified) |
| **Owner-only** | Yes | No | No | Bucket manageable but list/upload blocked |
| **Owner + Write, no Read** | Yes | No | Yes | Upload succeeds; listing fails; download blocked |
| **Owner + Read, no Write** | Yes | Yes | No | Same UX as read-only |

### Tested Permission Error Messages

| Operation | Blocked by | Error message displayed |
| --- | --- | --- |
| Upload (write: false) | Garage 403 | *"Access denied. You do not have permission to upload here."* |
| Delete (write: false) | Garage 403 | *"Access denied. You do not have permission to delete this item."* |
