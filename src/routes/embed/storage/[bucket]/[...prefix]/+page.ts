 ...
 * config from localStorage. Redirects to /embed/storage if no connection is available.
...
export const load: PageLoad = async ({ fetch, url, data }) => {
  ...
  if (!connection) throw redirect(303, '/embed/storage');
  ...
  if (!res.ok) {
    if (res.status === 401) throw redirect(303, '/embed/storage');
