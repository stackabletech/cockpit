<script lang="ts">
  ...
  // error (the error boundary sits at the parent /embed/storage level, and SvelteKit
  ...
  const bucket = $derived(
    page.params.bucket ||
      decodeURIComponent(page.url?.pathname?.split('/').filter(Boolean)[2] ?? '')
  );
...
>
  ...
  <div class="flex flex-wrap items-center justify-center gap-3">
    <a href={resolve('/embed/storage')} class="btn btn-sm btn-primary">
