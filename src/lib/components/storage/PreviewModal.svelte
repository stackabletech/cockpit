<script lang="ts">
  ...
  /**
   * Read response body as UTF-8 text. Returns null if the content cannot be decoded as UTF-8
   * (indicating binary content).
   */
  async function readTextSafely(res: Response): Promise<string | null> {
    try {
      const buf = await res.arrayBuffer();
      // Check for non-UTF-8 sequences by decoding with fatal:true
...
<Modal bind:open class="modal">
  <div
    ...
    style={maximized ? '' : 'max-height: min(90dvh, 900px)'}
  >
    <!-- Header -->
    ...
    <!-- Footer -->
    {#if preview.kind === 'text' || preview.kind === 'csv' || preview.kind === 'image' || preview.kind === 'pdf'}
