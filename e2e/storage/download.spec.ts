...
test.describe('Storage S3 — Download', () => {
  ...
  test('lists an archived download in the download history and can re-download it', async ({
    page
  }, testInfo) => {
    const credentials = requireGarageCredentials();
    const client = createS3Client(credentials);
    const prefix = uniquePrefix(testInfo, 'dl-history');
    const cleanupKeys = ['one.txt', 'two.txt', 'three.txt', 'four.txt'].map(
      (name) => `${prefix}${name}`
    );
    ...
    try {
      ...
      // The archived download is listed in the operations dropdown.
      await page.getByRole('button', { name: 'Operations' }).click();
      const menu = page.getByRole('menu', { name: 'Operations' });
      const history = page.getByRole('region', { name: 'Download history' });

      // The dropdown panel must be scrollable so long histories stay reachable.
      await expect(menu).toHaveCSS('overflow-y', 'auto');
      await expect(menu).toHaveCSS('max-height', /^(?!0px).*$/);
      await expect(history.getByText(prefix)).toBeVisible();
      await expect(history.getByText('4 files')).toBeVisible();
      await expect(history.getByText('ZIP')).toBeVisible();

      // Re-downloading from the history streams the archive again.
      await history.getByRole('button', { name: 'Show details' }).click();
      const [download] = await Promise.all([
        page.waitForEvent('download'),
        history.getByRole('button', { name: 'Download all' }).click()
      ]);
      expect(download.suggestedFilename()).toMatch(/\.zip$/);
    } finally {
      await deleteKnownKeys(client, credentials.bucket, cleanupKeys);
